...
if(!$conn){
	...
	if(!$result){
		...
		if(mysqli_num_rows($result)>0){
			while ($row=mysqli_fetch_assoc($result)) {
				...
				$values["eid"]=$row["eid"];
				//$values["uid"]=$row['uid'];
				$values["ename"]=$row["ename"];
				$values["edesc"]=$row["edesc"];
                $data[]=$values;
			}
	...
	break;
		...
		case 'post':
		//$uid=$postdata->uid;
		$ename=$postdata->ename;
		$edesc=$postdata->edesc;
		$query="insert into $db_name.$table_name (ename,edesc) values('$ename','$edesc')" ;
		$result=mysqli_query($conn,$query);
		if(!$result){
		$response["status"]="error";
